...
RUN_CONFIGS = {
    "full": {
        ...
        "run_name": "1.3 Full-History Advanced Boosting Optuna Tuning Diagnostic",
        "report_path": (
            REPORTS_DIR / "full_history_advanced_boosting_optuna_training_report.xlsx"
        ),
        "feature_list_path": (
            METADATA_DIR / "full_history_advanced_boosting_optuna_features.txt"
        ),
        "selected_model_path": (
            MODELS_DIR / "full_history_advanced_boosting_optuna_model.joblib"
        ),
        "figures_dir": FIGURES_BASE_DIR / "full_history_advanced_boosting_optuna",
        "run_type": "full_history_advanced_boosting_optuna_tuning",
        "training_info": (
            "1.3 Full-History Advanced Boosting Optuna Tuning Diagnostic. "
            "Tunes Random Forest, Extra Trees, Gradient Boosting, XGBoost, "
    ...
    "vif_auto_full_history": {
        "modeling_version": "1.3",
        "run_name": (
            "1.3 Full-History VIF-Reduced Advanced Boosting "
            "Optuna Tuning Diagnostic"
        ),
        "report_path": (
            REPORTS_DIR
            / "full_history_vif_advanced_boosting_optuna_training_report.xlsx"
        ),
        "feature_list_path": (
            METADATA_DIR / "full_history_vif_advanced_boosting_optuna_features.txt"
        ),
        "selected_model_path": (
            MODELS_DIR / "full_history_vif_advanced_boosting_optuna_model.joblib"
        ),
        "figures_dir": FIGURES_BASE_DIR / "full_history_vif_advanced_boosting_optuna",
        "run_type": "full_history_vif_advanced_boosting_optuna_tuning",
        "training_info": (
            "1.3 Full-History VIF-Reduced Advanced Boosting Optuna "
            "Tuning Diagnostic. Tunes Random Forest, Extra Trees, "
            "Gradient Boosting, XGBoost, CatBoost, and AdaBoost using "
            "the VIF-reduced full-history feature set."
        ),
    },
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feature-set",
        default="vif_auto_full_history",
        choices=sorted(RUN_CONFIGS.keys()),
        help=(
            "Named full-history feature set to use for Optuna tuning. "
            "Defaults to 'vif_auto_full_history' for the 1.3 advanced "
            "boosting experiment."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of Optuna trials to run. This is total trials, not trials per model.",
    )
...
def get_run_config(feature_set_name: str) -> dict:
...
def build_adaboost_model(
    ...
    """
    Build AdaBoost while supporting both newer and older scikit-learn versions.
...
def build_model_from_trial(trial: optuna.Trial):
    ...
    if model_type == "xgboost":
        return XGBRegressor(
            ...
            min_child_weight=trial.suggest_int("xgb_min_child_weight", 2, 15),
            subsample=trial.suggest_float(
                "xgb_subsample",
                0.6,
                1.0,
    ...
    if model_type == "catboost":
        return CatBoostRegressor(
            ...
            depth=trial.suggest_int("cat_depth", 2, 5),
    ...
    if model_type == "adaboost":
        base_estimator = DecisionTreeRegressor(
            max_depth=trial.suggest_int("ada_max_depth", 1, 3),
            min_samples_leaf=trial.suggest_int("ada_min_samples_leaf", 2, 15),
            random_state=33,
        )

        return build_adaboost_model(
...
def build_model_from_best_params(best_params: dict):
    ...
    if model_type == "adaboost":
        base_estimator = DecisionTreeRegressor(
            max_depth=best_params["ada_max_depth"],
            min_samples_leaf=best_params["ada_min_samples_leaf"],
            random_state=33,
        )
        ...
        return build_adaboost_model(
            ...
            learning_rate=best_params["ada_learning_rate"],
            loss=best_params["ada_loss"],
        )
...
def build_best_trials_by_model_table(study: optuna.study.Study) -> pd.DataFrame:
    ...
    for model_type, trial in best_trials_by_model.items():
        training_mae = trial.user_attrs.get("training_mae")
        ...
        record = {
            ...
            "training_r2": trial.user_attrs.get("training_r2"),
            "training_total_deviation_pct": trial.user_attrs.get(
                "training_total_deviation_pct"
            ),
        ...
        if training_mae is not None:
            record["mae_generalization_gap"] = validation_mae - training_mae
...
def build_run_summary_table(
    ...
    summary = {
        ...
        "candidate_model_types": ", ".join(MODEL_TYPES),
...
def log_run_to_mlflow(
    ...
    with mlflow.start_run(run_name=run_config["run_name"]) as run:
        ...
        mlflow.log_param("candidate_model_types", ",".join(MODEL_TYPES))
        ...
        for metric_name in metric_names:
            log_metric_if_valid(
                ...
                float(training_metrics[metric_name]),
            )
        ...
        log_artifact_if_exists(
            run_config["selected_model_path"],
            mlflow_artifact_path="models",
        )

        log_artifact_if_exists(
            run_config["feature_list_path"],
            mlflow_artifact_path="metadata",
        )

        figures_dir = run_config["figures_dir"]
...
def main(
    ...
    n_trials: int = 30,
    feature_set_name: str = "vif_auto_full_history",
) -> None:
    optuna.logging.set_verbosity(optuna.logging.WARNING)

    run_config = get_run_config(feature_set_name)
    ...
    print("Candidate models: " + ", ".join(MODEL_TYPES))
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trial.set_user_attr("training_mae", training_metrics["mae"])
        trial.set_user_attr("training_r2", training_metrics["r2"])
        trial.set_user_attr(
            "training_total_deviation_pct",
            training_metrics["total_deviation_pct"],
        )
    ...
    study = optuna.create_study(
        direction="minimize",
        sampler=sampler,
        study_name=f"full_history_optuna_tuning_{feature_set_name}_advanced_boosting",
    )
    ...
    training_predictions = best_model_for_validation.predict(X_train)
    training_metrics = calculate_regression_metrics(
        y_train,
        training_predictions,
    )
    ...
    print_metrics("FULL-HISTORY OPTUNA TRAINING DIAGNOSTIC METRICS", training_metrics)
    ...
    run_config["figures_dir"].mkdir(parents=True, exist_ok=True)
    METADATA_DIR.mkdir(parents=True, exist_ok=True)
    MODELS_DIR.mkdir(parents=True, exist_ok=True)

    save_feature_list(selected_feature_columns, run_config["feature_list_path"])

    with pd.ExcelWriter(run_config["report_path"], engine="openpyxl") as writer:
        ...
        pd.DataFrame([training_metrics]).to_excel(
            writer,
    ...
    joblib.dump(selected_model, run_config["selected_model_path"])
    ...
    print("\nSaved outputs:")
    print(f"- {run_config['report_path']}")
    print(f"- {run_config['selected_model_path']}")
    print(f"- {run_config['feature_list_path']}")
    print(f"- {run_config['figures_dir']}")
